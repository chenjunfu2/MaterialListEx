...
public:
  uint64_t DataHash(void) {
    static_assert(std::is_same_v<XXH64_hash_t, uint64_t>,
                  "Hash type does not match the required type.");
...
public:
  static size_t Hash(const ItemInfo &self) { return self.u64Hash; }
...
public:
  static size_t Hash(const NoTagItemInfo &self) { return self.u64Hash; }
...
public:
  ...
  */
  ...
  static ItemStackList
  ItemStackListUnpackContainer(ItemStackList &&listItemStack,
                               size_t szStackDepth = 64) {
    ItemStackList ret;
    ret.reserve(
        listItemStack
            .size()); // 提前扩容（但是解包的情况下可能还会添加新的东西）
  ...
  static ItemStack
  ItemCompoundToItemStack(const NBT_Node::NBT_Compound &cpdItem) {
    ...
    return ItemStack{sName == NULL ? NBT_Node::NBT_String{} : *sName,
                     cpdTag == NULL ? NBT_Node::NBT_Compound{} : *cpdTag,
                     byteCount == NULL ? uint64_t{}
                                       : (uint64_t)(uint8_t)*byteCount};
  }
  ...
  static ItemStack ItemCompoundToItemStack(NBT_Node::NBT_Compound &&cpdItem) {
    ...
    return ItemStack{
        ...
        byteCount == NULL
            ? uint64_t{}
            : (uint64_t)(uint8_t)*byteCount // 内置类型直接拷贝，移动个P
    };
  }
...
private:
  ...
  */
  ...
  static void ItemStackUnpackContainer(ItemStack &&stItemStack,
                                       ItemStackList &listItemStack,
                                       size_t szStackDepth) {
    if (szStackDepth <= 0) {
      return; // 栈深度超了，直接返回
    }
    ...
    if (stItemStack.sItemName.ends_with(MU8STR("shulker_box"))) {
      ...
      auto pcpdBlockEntityTag =
          stItemStack.cpdItemTag.HasCompound(MU8STR("BlockEntityTag"));
      if (pcpdBlockEntityTag == NULL) {
        goto push_return;
      }

      // 现在找内部的Items（潜影盒内部只能是这个，不用想了）
      auto pcpdItems = pcpdBlockEntityTag->Search(MU8STR(
          "Items")); // 这里不用HasXXX而是Search是为了不解包出类型，兼容TraversalItems参数
      TraversalItems(
          pcpdItems, listItemStack, stItemStack.u64ItemCount,
          szStackDepth); // 获取当前容器的物品个数作为容器内所有物品的倍率
    } else if (stItemStack.sItemName ==
               MU8STR("minecraft:bundle")) // 收纳袋特殊处理
    {
      // 直接找到Items
      auto plistItems = stItemStack.cpdItemTag.Search(MU8STR(
          "Items")); // 这里不用HasXXX而是Search是为了不解包出类型，兼容TraversalItems参数
      TraversalItems(plistItems, listItemStack, stItemStack.u64ItemCount,
                     szStackDepth);
    }
  ...
  static void TraversalItems(NBT_Node *pItems, ItemStackList &listItemStack,
                             uint64_t u64Scale, size_t szStackDepth) {
    if (pItems == NULL) {
      return; // 为空直接返回
    }
    ...
    if (tag == NBT_Node::TAG_Compound) {
      ...
      pItems->GetCompound()
          .clear(); // 清空（内部数据已经转移所有权，不应该再使用）

      // 对内部的物品乘以倍率缩放
      tmpItem.u64ItemCount *= u64Scale;
      ItemStackUnpackContainer(std::move(tmpItem), listItemStack,
                               szStackDepth - 1); // 递归 - 1
      ...
      {
        ItemStack tmpItem =
            ItemCompoundToItemStack(std::move(it.GetCompound()));
        // 这里先不清理
        // 对内部的物品乘以倍率缩放
        tmpItem.u64ItemCount *= u64Scale;
        ItemStackUnpackContainer(std::move(tmpItem), listItemStack,
