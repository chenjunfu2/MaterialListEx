...
RegionStatsList RegionProcess(const NBT_Node::NBT_Compound &cpRegions) {
  ...
  for (const auto &[RgName, RgVal] : cpRegions) // 遍历选区
  {
    ...
    // 方块到物品处理
    {
      ...
      auto listBlockStats =
          BlockProcess::GetBlockStats(RgCompound); // 获取方块统计列表
