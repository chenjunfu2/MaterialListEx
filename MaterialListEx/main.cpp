...
int main(int argc, char *argv[]) {
                          ...
                          sNbtData.size())) // 如果nbt已压缩，解压，否则保持原样
  {
    ...
    if (pTest != NULL) {
      ...
      timer.Start();
      if (fwrite(sNbtData.data(), sizeof(sNbtData[0]), sNbtData.size(),
                 pFile) != sNbtData.size()) {
        return -1;
      }
      timer.Stop();
  ...
  const auto &tmp =
      nRoot
          .GetCompound(); // 获取根下第一个compound，正常情况下根部下只有这一个compound
  if (tmp.size() != 1) {
  ...
  const auto &cpdRegions =
      GetCompound(root.second).GetCompound(MU8STR("Regions"));
  ...
  for (const auto &it : vtRegionStats) {
    ...
    printf("\n===========[entity inventory]===========\n");
    PrintInfo<Language::Item>(it.mslEntityInventory.listSort,
                              lang); // 实体物品栏
  }
