...
template <typename MU8T = char, typename U16T = char16_t> class MUTF8_Tool {
...
private:
  ...
  static void EncodeMUTF8Bmp(const U16T u16Char, MU8T (&mu8CharArr)[szBytes]) {
    if constexpr (szBytes == 1) {
      mu8CharArr[0] =
          (uint8_t)((((uint16_t)u16Char & (uint16_t)0b0000'0000'0111'1111) >>
                     0) |
                    (uint16_t)0b0000'0000); // 0 6-0   7bit
    } else if constexpr (szBytes == 2) {
      mu8CharArr[0] =
          (uint8_t)((((uint16_t)u16Char & (uint16_t)0b0000'0111'1100'0000) >>
                     6) |
                    (uint16_t)0b1100'0000); // 110 10-6  5bit
      mu8CharArr[1] =
          (uint8_t)((((uint16_t)u16Char & (uint16_t)0b0000'0000'0011'1111) >>
                     0) |
                    (uint16_t)0b1000'0000); // 10 5-0   6bit
    } else if constexpr (szBytes == 3) {
      mu8CharArr[0] =
          (uint8_t)((((uint16_t)u16Char & (uint16_t)0b1111'0000'0000'0000) >>
                     12) |
                    (uint16_t)0b1110'0000); // 1110 15-12   4bit
      mu8CharArr[1] =
          (uint8_t)((((uint16_t)u16Char & (uint16_t)0b0000'1111'1100'0000) >>
                     6) |
                    (uint16_t)0b1000'0000); // 10 11-6   6bit
      mu8CharArr[2] =
          (uint8_t)((((uint16_t)u16Char & (uint16_t)0b0000'0000'0011'1111) >>
                     0) |
                    (uint16_t)0b1000'0000); // 10 5-0   6bit
  ...
  static void EncodeMUTF8Supplementary(const U16T u16HighSurrogate,
                                       const U16T u16LowSurrogate,
                                       MU8T (&mu8CharArr)[6]) {
    // 取出代理对数据并组合 范围：100000-1FFFFF
    uint32_t u32RawChar = //(uint32_t)0b0000'0000'0001'0000'0000'0000'0000'0000
                          //|//U16代理对实际编码高位 bit20 -> 1 ignore
        ((uint32_t)((uint16_t)u16HighSurrogate &
                    (uint16_t)0b0000'0011'1111'1111))
            << 10 | // 10bit
        ((uint32_t)((uint16_t)u16LowSurrogate &
                    (uint16_t)0b0000'0011'1111'1111))
            << 0; // 10bit
    ...
    mu8CharArr[1] =
        (uint8_t)(((u32RawChar &
                    (uint32_t)0b0000'0000'0000'1111'0000'0000'0000'0000) >>
                   16) |
                  (uint32_t)0b1010'0000); // 1010 19-16(20固定为1)   4bit
    mu8CharArr[2] =
        (uint8_t)(((u32RawChar &
                    (uint32_t)0b0000'0000'0000'0000'1111'1100'0000'0000) >>
                   10) |
                  (uint32_t)0b1000'0000); // 10 15-10   6bit
    // 低代理
    mu8CharArr[3] = (uint8_t)0b1110'1101; // 固定字节
    mu8CharArr[4] =
        (uint8_t)(((u32RawChar &
                    (uint32_t)0b0000'0000'0000'0000'0000'0011'1100'0000) >>
                   6) |
                  (uint32_t)0b1011'0000); // 1011 9-6   4bit
    mu8CharArr[5] =
        (uint8_t)(((u32RawChar &
                    (uint32_t)0b0000'0000'0000'0000'0000'0000'0011'1111) >>
                   0) |
                  (uint32_t)0b1000'0000); // 10 5-0   6bit
  }

  template <size_t szBytes>
  static void DecodeMUTF8Bmp(const MU8T (&mu8CharArr)[szBytes], U16T &u16Char) {
    if constexpr (szBytes == 1) {
      u16Char = ((uint16_t)((uint8_t)mu8CharArr[0] & (uint8_t)0b0111'1111))
                << 0; // 7bit
    } else if constexpr (szBytes == 2) {
      u16Char = ((uint16_t)((uint8_t)mu8CharArr[0] & (uint8_t)0b0001'1111))
                    << 6 | // 5bit
                ((uint16_t)((uint8_t)mu8CharArr[1] & (uint8_t)0b0011'1111))
                    << 0; // 6bit
    } else if constexpr (szBytes == 3) {
      u16Char = ((uint16_t)((uint8_t)mu8CharArr[0] & (uint8_t)0b0000'1111))
                    << 12 | // 4bit
                ((uint16_t)((uint8_t)mu8CharArr[1] & (uint8_t)0b0011'1111))
                    << 6 | // 6bit
                ((uint16_t)((uint8_t)mu8CharArr[2] & (uint8_t)0b0011'1111))
                    << 0; // 6bit
    } else {
      static_assert(false, "Error szBytes Size"); // 大小错误
    }
  ...
  static void DecodeMUTF8Supplementary(const MU8T (&mu8CharArr)[6],
                                       U16T &u16HighSurrogate,
                                       U16T &u16LowSurrogate) {
    // 忽略mu8CharArr[0]和mu8CharArr[3]（固定字节）
    uint32_t u32RawChar = //(uint16_t)0b0000'0000'0001'0000'0000'0000'0000'0000
                          //|//bit20->1 ignore mu8CharArr[0] ignore
        ((uint16_t)((uint8_t)mu8CharArr[1] & (uint8_t)0b0000'1111))
            << 16 | // 4bit
        ((uint16_t)((uint8_t)mu8CharArr[2] & (uint8_t)0b0011'1111))
            << 10 | // 6bit
                    // mu8CharArr[3] ignore
        ((uint16_t)((uint8_t)mu8CharArr[4] & (uint8_t)0b0000'1111))
            << 6 | // 4bit
        ((uint16_t)((uint8_t)mu8CharArr[5] & (uint8_t)0b0011'1111))
            << 0; // 6bit
    ...
    u16HighSurrogate =
        (uint32_t)((u32RawChar &
                    (uint32_t)0b0000'0000'0000'1111'1111'1100'0000'0000) >>
                       10 |
                   (uint32_t)0b1101'1000'0000'0000);
    u16LowSurrogate =
        (uint32_t)((u32RawChar &
                    (uint32_t)0b0000'0000'0000'0000'0000'0011'1111'1111) >>
                       0 |
                   (uint32_t)0b1101'1100'0000'0000);
  }

public:
// v=val b=beg e=end 注意范围是左右边界包含关系，而不是普通的左边界包含
#define IN_RANGE(v, b, e)                                                      \
  ((uint16_t)(v) >= (uint16_t)(b) && (uint16_t)(v) <= (uint16_t)(e))
  static std::basic_string<MU8T>
  U16ToMU8(const std::basic_string<U16T> &u16String) {
    ...
    for (auto it = u16String.begin(), end = u16String.end(); it != end; ++it) {
      ...
      } else if (IN_RANGE(u16Char, 0x0080, 0x07FF) ||
                 u16Char == 0x0000) // 双字节码点，0字节特判
      {
        MU8T mu8Char[2];
        EncodeMUTF8Bmp(u16Char, mu8Char);
        mu8String.append(mu8Char, sizeof(mu8Char) / sizeof(MU8T));
      } else if (IN_RANGE(u16Char, 0x0800, 0xFFFF)) // 三字节码点or多字节码点
      {
        if (IN_RANGE(u16Char, 0xD800, 0xDBFF)) // 遇到高代理对
        {
          ...
          if (!IN_RANGE(u16Char, 0xDC00, 0xDFFF)) {
            ...
            MU8T mu8Char[6];
            EncodeMUTF8Supplementary(u16HighSurrogate, u16LowSurrogate,
                                     mu8Char);
            mu8String.append(mu8Char, sizeof(mu8Char) / sizeof(MU8T));
          }
        } else {
...
#define GET_NEXTCHAR(c)                                                        \
  if (++it == end) {                                                           \
    break;                                                                     \
  } else {                                                                     \
    c = *it;                                                                   \
  }
  static std::basic_string<U16T>
  MU8ToU16(const std::basic_string<MU8T> &mu8String) {
    ...
    for (auto it = mu8String.begin(), end = mu8String.end(); it != end; ++it) {
      MU8T mu8Char = *it;
      // 判断是几字节的mu8
      if (((uint8_t)mu8Char & (uint8_t)0b1000'0000) ==
          (uint8_t)0b0000'0000) // 最高位不为1，单字节码点
      {
        // 放入数组
      ...
      } else if (((uint8_t)mu8Char & (uint8_t)0b1110'0000) ==
                 (uint8_t)0b1100'0000) // 高3位中最高两位为1，双字节码点
      {
        // 先保存第一个字节
        ...
        // 判断字节合法性
        if (((uint8_t)mu8Char & (uint8_t)0b1100'0000) !=
            (uint8_t)0b1000'0000) // 高2位不是10，错误，跳过
        {
        ...
        DecodeMUTF8Bmp(mu8CharArr, u16Char);
        u16String.push_back(u16Char);
      } else if (((uint8_t)mu8Char & (uint8_t)0b1111'0000) ==
                 (uint8_t)0b1110'0000) // 高4位为1110，三字节或多字节码点
      {
        // 保存
        ...
        // 把他们合起来就是1101'10xx
        // 也就是0xD8，即u16的高代理对开始字符，而代理对在encode过程走的另一个流程，不存在与3字节码点混淆处理的情况
        if (mu8First == (uint8_t)0b1110'1101 &&
            ((uint8_t)mu8Char & (uint8_t)0b1111'0000) ==
                (uint8_t)0b1010'0000) // 代理对，必须先判断，很重要！
        {
          MU8T mu8CharArr[6] = {
              mu8First,
              mu8Char}; // 0 1已初始化，0是固定起始值，1是高代理的高4位
          // 继续读取后4个并验证
          ...
          // 下一个为低代理高4位
          GET_NEXTCHAR(mu8Char);
          if (((uint8_t)mu8Char & (uint8_t)0b1111'0000) !=
              (uint8_t)0b1011'0000) {
          ...
          DecodeMUTF8Supplementary(mu8CharArr, u16HighSurrogate,
                                   u16LowSurrogate);
          u16String.push_back(u16HighSurrogate);
          u16String.push_back(u16LowSurrogate);
        } else if (((uint8_t)mu8Char & (uint8_t)0b1100'0000) ==
                   (uint8_t)0b1000'0000) // 三字节码点，排除代理对后只有这个可能
        {
          // 保存
          MU8T mu8CharArr[3] = {mu8First, mu8Char};
          GET_NEXTCHAR(mu8Char); // 尝试获取下一字符
          if (((uint8_t)mu8Char & (uint8_t)0b1100'0000) !=
              (uint8_t)0b1000'0000) // 错误，3字节码点最后一个不是正确字符
          {
            --it; // 撤回一次读取（为什么不是两次？因为前一个字符已确认高2bit为10，没有以10开头的存在，跳过）
            continue;
          }
...
codespace can be represented. Modified UTF-8 strings are not null-terminated.
...
UTF-16 representation. Each of the surrogate code units is represented by three
bytes. This means supplementary characters are represented by six bytes, u, v,
w, x, y, and z :
...
The Java Virtual Machine does not recognize the four-byte format of standard
...
Unicode Encoding Forms of The Unicode Standard, Version 13.0.
*/
...
字符串内容使用修改后的 UTF-8 编码。改进的 UTF-8 编码使得仅包含非空 ASCII
字符的代码点序列可以用每代码点 1 字节表示，同时能表示 Unicode
编码空间的所有代码点。修改后的 UTF-8 字符串不以空字符结尾。编码规则如下：
...
U+FFFF 以上的代码点（补充字符）通过分别编码其 UTF-16
表示的两个代理代码单元实现。每个代理代码单元用三个字节表示，即补充字符最终由六个字节
u, v, w, x, y 和 z 表示：
...
关于标准UTF-8格式的更多信息，请参阅《The Unicode Standard,
Version 13.0》第3.9节"Unicode Encoding Forms"。
*/