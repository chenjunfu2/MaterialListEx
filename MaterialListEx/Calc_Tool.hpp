...
template <typename T> const T &Max(const T &l, const T &r) {
  return (l > r) ? l : r;
}

template <typename T> const T &Min(const T &l, const T &r) {
  return (l < r) ? l : r;
}
...
static inline BlockPos
getRelativeEndPositionFromAreaSize(const BlockPos &size) {
  auto x = size.x;
  auto y = size.y;
  auto z = size.z;
